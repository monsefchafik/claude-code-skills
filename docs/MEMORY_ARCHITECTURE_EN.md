...
| Placeholder rows (`_To be filled_`) | Zero signal, erodes trust | Remove |
...
| `docs/MEMORY_ARCHITECTURE_EN.md` | ✓ | — | — | — | — | — | — |
...
│   ├── MEMORY_ARCHITECTURE_EN.md              ← This file — memory system contract
...
and that `memory-maintainer` marks as `<!-- To be filled -->` when it cannot fill them.
...
Project start (once)
  └─ /memory-bootstrap
       ...
       → human completes the "To be filled" sections
...
- [ ] Complete the `To be filled` sections (business rules, domain terms, operational pitfalls)
...
- `docs/MEMORY_ARCHITECTURE_EN.md` — adoption documentation, never updated by the skill
